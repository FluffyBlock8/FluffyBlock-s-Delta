...
(function (global, factory) {
  ...
  */
  ...
  if (typeof window !== 'undefined') {
    var samAudioContext = null;

    function getSamAudioContext() {
      if (samAudioContext === null) {
        samAudioContext = new (window.AudioContext || window.webkitAudioContext)();
      }
      return samAudioContext;
    }

    function buildAudioSource(buffer) {
      var context = getSamAudioContext();
      var source = context.createBufferSource();
      var soundBuffer = context.createBuffer(1, buffer.length, 22050);
      soundBuffer.getChannelData(0).set(buffer);
      source.buffer = soundBuffer;
      source.connect(context.destination);
      return source;
    }

    function voiceToOptions(voice) {
      var opts = {};
      if (!voice) return opts;
      if (typeof voice === 'object') return voice;
      if (typeof voice !== 'string') return opts;

      var name = voice.toLowerCase();
      if (name.indexOf('high') !== -1 || name.indexOf('female') !== -1) {
        opts.pitch = 90;
        opts.mouth = 140;
      } else if (name.indexOf('low') !== -1 || name.indexOf('male') !== -1) {
        opts.pitch = 40;
        opts.mouth = 100;
      }
      if (name.indexOf('fast') !== -1) {
        opts.speed = 100;
      } else if (name.indexOf('slow') !== -1) {
        opts.speed = 60;
      }
      return opts;
    }
    ...
    window.speak.play = function (text, self, voice, onended, onstart) {
      if (window.tts == null) window.tts = {};
      if (window.tts[self] != null && window.tts[self].started) {
        window.tts[self].stop();
      } else if (window.tts[self] != null) {
        window.tts[self].start = function () {};
        window.tts[self].started = false;
      }

      var sam = new SamJs(voiceToOptions(voice));
      var buffer = sam.buf32(text);
      var source = buildAudioSource(buffer);
      ...
      source.onended = function () {
        ...
        if (window.tts[self] === source) delete window.tts[self];
      };
      ...
      function startSource() {
        ...
        if (typeof onstart === 'function') onstart(source);
      }

      startSource();
      source.endTimeout = setTimeout(function () {
        if (window.tts[self] === source) {
          delete window.tts[self];
          if (typeof onended === 'function') onended();
        }
      }, Math.ceil((buffer.length / 22050) * 1000) + 100);
